...
public class AABB {
  ...
  /**
   * Calculates the intersection point between a ray and this AABB.
   * @param start The start point of the ray (eye position)
   * @param end The end point of the ray (eye + direction * distance)
   * @return The intersection point, or null if no intersection
   */
  public double[] calculateIntercept(double startX, double startY, double startZ,
                                     double endX, double endY, double endZ) {
    double[] minXIntercept = getIntermediateWithXValue(startX, startY, startZ, endX, endY, endZ, this.minX);
    double[] maxXIntercept = getIntermediateWithXValue(startX, startY, startZ, endX, endY, endZ, this.maxX);
    double[] minYIntercept = getIntermediateWithYValue(startX, startY, startZ, endX, endY, endZ, this.minY);
    double[] maxYIntercept = getIntermediateWithYValue(startX, startY, startZ, endX, endY, endZ, this.maxY);
    double[] minZIntercept = getIntermediateWithZValue(startX, startY, startZ, endX, endY, endZ, this.minZ);
    double[] maxZIntercept = getIntermediateWithZValue(startX, startY, startZ, endX, endY, endZ, this.maxZ);

    if (!isYZInside(minXIntercept)) minXIntercept = null;
    if (!isYZInside(maxXIntercept)) maxXIntercept = null;
    if (!isXZInside(minYIntercept)) minYIntercept = null;
    if (!isXZInside(maxYIntercept)) maxYIntercept = null;
    if (!isXYInside(minZIntercept)) minZIntercept = null;
    if (!isXYInside(maxZIntercept)) maxZIntercept = null;

    double[] result = null;

    if (minXIntercept != null) {
      result = minXIntercept;
    }
    if (maxXIntercept != null && (result == null || distSq(startX, startY, startZ, maxXIntercept) < distSq(startX, startY, startZ, result))) {
      result = maxXIntercept;
    }
    if (minYIntercept != null && (result == null || distSq(startX, startY, startZ, minYIntercept) < distSq(startX, startY, startZ, result))) {
      result = minYIntercept;
    }
    if (maxYIntercept != null && (result == null || distSq(startX, startY, startZ, maxYIntercept) < distSq(startX, startY, startZ, result))) {
      result = maxYIntercept;
    }
    if (minZIntercept != null && (result == null || distSq(startX, startY, startZ, minZIntercept) < distSq(startX, startY, startZ, result))) {
      result = minZIntercept;
    }
    if (maxZIntercept != null && (result == null || distSq(startX, startY, startZ, maxZIntercept) < distSq(startX, startY, startZ, result))) {
      result = maxZIntercept;
    }

    return result;
  }

  private double distSq(double x, double y, double z, double[] point) {
    double dx = point[0] - x;
    double dy = point[1] - y;
    double dz = point[2] - z;
  ...
  private double[] getIntermediateWithXValue(double startX, double startY, double startZ,
                                              double endX, double endY, double endZ, double x) {
    double dx = endX - startX;
    double dy = endY - startY;
    double dz = endZ - startZ;

    if (dx * dx < PRECISION_THRESHOLD) return null;

    double t = (x - startX) / dx;
    if (t < 0.0 || t > 1.0) return null;

    return new double[]{x, startY + dy * t, startZ + dz * t};
  }

  private double[] getIntermediateWithYValue(double startX, double startY, double startZ,
                                              double endX, double endY, double endZ, double y) {
    double dx = endX - startX;
    double dy = endY - startY;
    double dz = endZ - startZ;

    if (dy * dy < PRECISION_THRESHOLD) return null;

    double t = (y - startY) / dy;
    if (t < 0.0 || t > 1.0) return null;

    return new double[]{startX + dx * t, y, startZ + dz * t};
  }

  private double[] getIntermediateWithZValue(double startX, double startY, double startZ,
                                              double endX, double endY, double endZ, double z) {
    double dx = endX - startX;
    double dy = endY - startY;
    double dz = endZ - startZ;

    if (dz * dz < PRECISION_THRESHOLD) return null;

    double t = (z - startZ) / dz;
    if (t < 0.0 || t > 1.0) return null;

    return new double[]{startX + dx * t, startY + dy * t, z};
  }

  private boolean isYZInside(double[] point) {
    if (point == null) return false;
    return point[1] >= this.minY && point[1] <= this.maxY && point[2] >= this.minZ && point[2] <= this.maxZ;
  }

  private boolean isXZInside(double[] point) {
    if (point == null) return false;
    return point[0] >= this.minX && point[0] <= this.maxX && point[2] >= this.minZ && point[2] <= this.maxZ;
  }

  private boolean isXYInside(double[] point) {
    if (point == null) return false;
    return point[0] >= this.minX && point[0] <= this.maxX && point[1] >= this.minY && point[1] <= this.maxY;
  }
}
